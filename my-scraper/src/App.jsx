...
function App() {
  ...
  const checkUrl = async (url) => {
    try {
      const response = await fetch(
        `${BACKEND_URL}/check-url?url=${encodeURIComponent(url)}`
      );
  ...
  const getCachedData = async (url) => {
    try {
      const response = await fetch(
        `${BACKEND_URL}/get-cached?url=${encodeURIComponent(url)}`
      );
  ...
  const searchByTags = async (tags) => {
    try {
      const response = await fetch(
        `${BACKEND_URL}/search-by-tags?tags=${encodeURIComponent(tags)}`
      );
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative flex gap-2">
            ...
            <input
              ...
              placeholder={
                isSearchMode ? "Enter tags to search" : "Enter URL to scrape"
              }
              ...
              <div className="space-y-4">
                <div className="mb-4">
                  ...
                  {results.searchResults.map((result) => (
                    <div
                      key={result.id}
                      className={`mb-4 p-4 rounded border-2 transition-all duration-300
                                                 ${
                                                   darkMode
